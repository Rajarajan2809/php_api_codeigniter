...
class Trigger extends REST_Controller
{
	...
	public function create_post()
	{
		...
		if(isset($json_data['type']))
		{
			...
			{
                foreach($json_data['trigger_input'] as $value1)
                {
                    $data = array(
                                    'trigger_name'      => $json_data['name'],
                                    'trigger_type_id'   => $json_data['type'],
			...
			foreach($json_data['parameters'] as $value1)
			{
		...
		if(isset($json_data['type']))
		{
			if($json_data['type'] == 'scene')
			{
				...
				if($query1->result())
				{
					...
					{
						$this->db->select('id');
                        $this->db->from('Action_Detail');
                        $this->db->where('action_id', $row1->action_id);
                        $query2 = $this->db->get();
                        foreach ($query2->result() as $row2)
                        {
                            $this->db->delete('Action_Detail', array('id' => $row2->id));
                        }

                        $this->db->select('id');
                        $this->db->from('Action_Notifications');
                        $this->db->where('action_id', $row3->action_id);
                        $query3 = $this->db->get();
                        foreach ($query3->result() as $row3)
                        {
                            $this->db->delete('Action_Notifications', array('id' => $row3->id));
                        }
