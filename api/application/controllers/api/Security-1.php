...
class Security extends REST_Controller 
{
	...
	# /security/arm
	public function arm_put($mode_id,$node_id){
			...
			$sock_data	=	$this->nodeSecurityInfo($node_id);
			$this->sent2socket(8286, json_encode($sock_data));
	...
	# /security/deactivate
	public function bypass_put($mode_id,$node_id){
			
			$data		=	array("enable_status"=>"0");
			$condition	=	array("mode_id"=>$mode_id,"node_id"=>$node_id);
...
function nodeSecurityInfo($node_id){
		$data		=	array("mode_id"=>"1","node_id"=>$node_id);
